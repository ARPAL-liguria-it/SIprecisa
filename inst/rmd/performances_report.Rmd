...
data_caption <- "Misure per l'analita %s."
summary_caption <- "Statistiche di base per l'analita %s."
...
for (i in ordered_analytes) {
  ...
  if("trueness" %in% params$report04$content) {
    ...
    cat(params$estimate03[[i]]$trueness)
    cat('\n\n<!-- -->\n\n')
  
  # detecting bias by t-test or E number
  ttest_title <- ifelse(params$estimate03[[i]]$refuncertainty == 0,
                        "Test per la significatività del bias (t-test)",
                        "Test per la significatività del bias ($E_n$)")
  cat(paste("###", ttest_title, "\n"))
  cat(params$estimate03[[i]]$ttest)
  cat('\n\n<!-- -->\n\n')
